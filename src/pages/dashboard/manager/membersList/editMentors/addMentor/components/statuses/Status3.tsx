...
import MentorInputs from "./status3/MentorInputs";
import PUserData from "../../../../../../../../api/common/PUserData";
import { useEffect } from "react";
import { useQueryClient } from "@tanstack/react-query";
import { useNavigate } from "react-router-dom";
import TopElems from "./status3/TopElems";

type Status3Props = {
   username: string,
}

function Status3({ username }: Status3Props) {
   const { handleSubmit, register, setValue, getValues, formState: { errors } } = useForm();

   const { mutate: addMentor, isPending: addMentorPending, isError: addMentorError, isSuccess: addMentorSuccess } = managerPAddMentor();
   ...
   return (
      <>
         <TopElems addMentorError={addMentorError} setInfoError={setInfoError} />
         ...
         <form onSubmit={handleSubmit(submitHandler)} className="mt-4">
            ...
            <div className="flex gap-4 items-center mt-12">
               <Btn
                  ...
                  isDisabled={addMentorPending || setInfoPending}
               />
            </div>
         </form>
      </>
