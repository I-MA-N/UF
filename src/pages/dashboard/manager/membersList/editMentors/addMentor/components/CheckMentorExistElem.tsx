...
import managerPMentorExist, { MentorExistRes } from "../../../../../../../api/manager/mangaerPMentorExist";
import validateUsername from "../../../../../../../utils/validateUsername";
import { useUserDataContext } from "../../../../../../authentication/UserDataProvider";
import { useMemo } from "react";
import splitArr from "../../../../../../../utils/splitArr";

type CheckMentorExistElemProps = {
   setMentorStatus: React.Dispatch<React.SetStateAction<MentorExistRes | null>>
}

function CheckMentorExistElem({ setMentorStatus }: CheckMentorExistElemProps) {
   const { orgNames } = useUserDataContext();
   const orgName = useMemo(() => splitArr(orgNames)[0], []);
   
   const { handleSubmit, register, formState: { errors } } = useForm<{ username: string }>()
   const { mutateAsync } = managerPMentorExist();

   const submitHandler = (data: { username: string }) => {
      if (data.username) {
         mutateAsync({ orgName, username: data.username })
            .then(res => res.status && setMentorStatus(res))
      }
   }
   ...
   return (
      <div>
         <div className="flex items-center gap-2">
            ...
            <Link
               to="/manager/dashboard/memberslist/editmentors"
         ...
         <form onSubmit={handleSubmit(submitHandler)} className="flex items-end gap-1 h-12 lg:h-14">
            <input
               ...
               {...register("username", { onChange: () => setMentorStatus(null), required: true, validate: validateUsername })}
            />
            <button
               type="submit"
...
export default CheckMentorExistElem