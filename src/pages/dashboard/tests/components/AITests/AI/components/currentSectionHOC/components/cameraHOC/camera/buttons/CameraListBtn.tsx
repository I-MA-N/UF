...
type CameraListBtnProps = {
   isLoading: boolean,
   resetCameraStates: () => void
}

function CameraListBtn({ isLoading, resetCameraStates }: CameraListBtnProps) {
   ...
   return (
      <>
         <button
            ...
            className={`
               ${!isLoading ? "bg-primary text-white border-white" : "bg-gray text-primary border-gray"}
               size-11 lg:size-14 border-2 rounded-full flex items-center justify-center transition-transform duration-200
            `}
         >
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 23 22" fill="none" className="size-6 lg:size-8">
...
export default memo(CameraListBtn);