...
function NeededTestNamesText() {
   const { program } = useParams();
   const neededTestNames = useMemo(() => (
      program === "fitness"
         ? ["مقدار تحرک", "وضعیت بدنی"]
         : program === "corrective"
            ? ["مقدار تحرک", "ناهنجاری ها", "ارزیابی پویا"]
            : null
   ), [program]);

   if (neededTestNames) return (
      <div className="flex items-center justify-center text-xs lg:text-sm text-center">
         <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="none" className="w-4.5 lg:w-6">
         ...
         {
            neededTestNames.map((testName, index) => (
               <span className="text-yellow">
                  {testName + (index + 1 !== neededTestNames.length ? "، " : "")}
               </span>
            ))
         }
      </div>
...
export default NeededTestNamesText;