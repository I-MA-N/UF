...
function ChangeDataModal({ setChangeModal }: ChangeDataModalProps) {
   const {
      register,
      handleSubmit,
      formState: { errors }
   ...
   return (
      <div className="modal">
         <button onClick={() => setChangeModal(false)} className="flex gap-0.5 items-center mb-4.5 bg-primary text-yellow lg:text-lg py-2 px-4 lg:px-5 rounded-[32px]">
         ...
         <form onSubmit={handleSubmit(submitHandler)} className="w-[298px] xs:w-80 lg:w-[424px] bg-white text-primary rounded-[32px] p-12">
            <div className="flex gap-3.5 items-center border-b border-primary pb-4 lg:pb-6">
            ...
            <div className="flex flex-col gap-3 pt-6 lg:pt-8 p-4">
               <ModalItem
            ...
            {
               isError &&
               <span className="block text-xs lg:text-sm text-center text-red mt-3">مشکلی در تغییر اطلاعات بوجود آمده است!</span>
            }
            {
               isSuccess &&
               <span className="block text-xs lg:text-sm text-center text-secondary mt-3">تغییرات با موفقیت انجام شد!</span>
            }
            <Btn
               text="تایید"
               type="submit"
