...
function UserListElem({ user, setState, setStateValue, isSimpleuser }: UserListElemProps) {
   return (
      <li
         key={user.username}
         className="flex items-center justify-between h-9 lg:h-11 text-[10px] lg:text-sm bg-white text-primary cursor-pointer"
         onClick={() => setState(setStateValue)}
      >
         ...
         <div className="flex justify-center items-center gap-1.5 px-2.5">
            <svg className={`w-4 lg:w-5 ${(isSimpleuser || user.age) && user.name && user.phone && 'text-secondary'}`} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 13 12" fill="none">
