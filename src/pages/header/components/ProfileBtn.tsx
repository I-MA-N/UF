   ...
   return (
      <NavLink
         ...
         className={({ isActive }: SetStyleFuncProps) => {
            setActive(isActive);
            return `flex items-center justify-center gap-1 md:gap-2 navlink ${active ? "font-Estedad-Black before:opacity-100" : ""}`;
         }}
      >
         <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 15 16" fill="none" className="w-4 lg:w-5 font-bold">
            ...
            <circle cx="4" cy="4" r="3.4" transform="matrix(-1 0 0 1 11.3334 0)" stroke="currentColor" strokeWidth={`${active ? '2' : '1.2'}`} />
         </svg>
         پروفایل
      </NavLink>
   )
