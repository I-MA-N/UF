...
function calorieAmountData({ data, gender }: PhysicalIndexPropsType) {
   dataCalorieAmount.BMI = data['وزن'] / Math.pow(data['قد'] / 100, 2);
   if (gender === 1) {
      dataCalorieAmount.metabolismBasic = 9.247 * data['وزن'] + 3.098 * data['قد'] - 4.33 * data['سن'] + 447.593;
   } else {
      dataCalorieAmount.metabolismBasic = 13.397 * data['وزن'] + 4.799 * data['قد'] - 5.677 * data['سن'] + 88.362;
   }
